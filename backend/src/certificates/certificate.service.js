"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.CertificateService = void 0;
var common_1 = require("@nestjs/common");
var typeorm_1 = require("typeorm");
var certificate_entity_1 = require("./entity/certificate.entity");
var christian_entity_1 = require("../christian/entity/christian.entity");
var CertificateService = /** @class */ (function () {
    function CertificateService(response) {
        this.response = response;
    }
    CertificateService.prototype.createCertificate = function (data) {
        return __awaiter(this, void 0, void 0, function () {
            var certificate, victim, data_1, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        certificate = new certificate_entity_1.Certificate();
                        return [4 /*yield*/, christian_entity_1.Christian.findOne({ where: { id: data } })];
                    case 1:
                        victim = _a.sent();
                        certificate.victim = victim;
                        certificate.isAllowed = false;
                        certificate.status = 1;
                        certificate.created_at = new Date();
                        certificate.updated_at = new Date();
                        certificate.created_by = 1;
                        certificate.updated_by = 1;
                        _a.label = 2;
                    case 2:
                        _a.trys.push([2, 4, , 5]);
                        return [4 /*yield*/, certificate.save()];
                    case 3:
                        data_1 = _a.sent();
                        return [2 /*return*/, this.response.postResponse(data_1.id)];
                    case 4:
                        error_1 = _a.sent();
                        console.log(error_1);
                        throw new common_1.InternalServerErrorException("something wrong : ", error_1);
                    case 5: return [2 /*return*/];
                }
            });
        });
    };
    CertificateService.prototype.updateCertificate = function (id, data) {
        return __awaiter(this, void 0, void 0, function () {
            var certificate, victim, data_2, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, certificate_entity_1.Certificate.findOne({
                            where: { id: id }
                        })];
                    case 1:
                        certificate = _a.sent();
                        if (!certificate)
                            throw new common_1.BadRequestException("This certificate ".concat(id, " not found"));
                        return [4 /*yield*/, christian_entity_1.Christian.findOne({ where: { id: data.victim } })];
                    case 2:
                        victim = _a.sent();
                        if (!victim)
                            throw new common_1.BadRequestException("This victim ".concat(id, " not found"));
                        certificate.victim = victim;
                        certificate.isAllowed = data.isAllowed;
                        certificate.status = 1;
                        certificate.created_at = new Date();
                        certificate.updated_at = new Date();
                        certificate.created_by = 1;
                        certificate.updated_by = 1;
                        _a.label = 3;
                    case 3:
                        _a.trys.push([3, 5, , 6]);
                        return [4 /*yield*/, certificate_entity_1.Certificate.update(id, certificate)];
                    case 4:
                        data_2 = _a.sent();
                        return [2 /*return*/, this.response.updateResponse(id)];
                    case 5:
                        error_2 = _a.sent();
                        throw new common_1.InternalServerErrorException("something wrong : ", error_2);
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    CertificateService.prototype.getAllCertificate = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, certificate_entity_1.Certificate.find({ where: { status: (0, typeorm_1.Not)(8) } })];
            });
        });
    };
    CertificateService.prototype.getSingleCertificate = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var certificate;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, certificate_entity_1.Certificate.findOne({
                            where: { status: (0, typeorm_1.Not)(8), id: id }
                        })];
                    case 1:
                        certificate = _a.sent();
                        if (!certificate)
                            throw new common_1.BadRequestException("This certificate ".concat(id, " not found"));
                        return [2 /*return*/, certificate];
                }
            });
        });
    };
    CertificateService.prototype.deleteCertificate = function (id, userId) {
        return __awaiter(this, void 0, void 0, function () {
            var certificate, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, certificate_entity_1.Certificate.findOne({
                            where: { status: (0, typeorm_1.Not)(8), id: id }
                        })];
                    case 1:
                        certificate = _a.sent();
                        if (!certificate)
                            throw new common_1.BadRequestException("This certificate ".concat(id, " not found"));
                        _a.label = 2;
                    case 2:
                        _a.trys.push([2, 4, , 5]);
                        certificate.status = 8;
                        certificate.updated_by = userId;
                        return [4 /*yield*/, certificate_entity_1.Certificate.update(id, certificate)];
                    case 3:
                        _a.sent();
                        return [2 /*return*/, this.response.deleteResponse(id)];
                    case 4:
                        error_3 = _a.sent();
                        throw new common_1.InternalServerErrorException("something wrong : ", error_3);
                    case 5: return [2 /*return*/];
                }
            });
        });
    };
    CertificateService = __decorate([
        (0, common_1.Injectable)()
    ], CertificateService);
    return CertificateService;
}());
exports.CertificateService = CertificateService;
